 ...
 * Generates standards-compliant Model 2 QR codes in pure JavaScript.
 * Supports Error Correction (L, M, Q, H), Byte Mode (UTF-8), Canvas & SVG output.
 */
(function(root) {
  ...
  var QRUtil = {
    ...
    getLostPoint: function(qrCode) {
      ...
      for (var r2 = 0; r2 < moduleCount - 1; r2++) {
        for (var c2 = 0; c2 < moduleCount - 1; c2++) {
          var count = 0;
          if (qrCode.isDark(r2, c2)) count++;
          if (qrCode.isDark(r2 + 1, c2)) count++;
          if (qrCode.isDark(r2, c2 + 1)) count++;
          if (qrCode.isDark(r2 + 1, c2 + 1)) count++;
          if (count == 0 || count == 4) lostPoint += 3;
        }
      }
      for (var r3 = 0; r3 < moduleCount; r3++) {
        for (var c3 = 0; c3 < moduleCount - 6; c3++) {
          if (qrCode.isDark(r3, c3) && !qrCode.isDark(r3, c3 + 1) && qrCode.isDark(r3, c3 + 2) &&
              qrCode.isDark(r3, c3 + 3) && qrCode.isDark(r3, c3 + 4) && !qrCode.isDark(r3, c3 + 5) &&
              qrCode.isDark(r3, c3 + 6)) lostPoint += 40;
        }
      }
      for (var c4 = 0; c4 < moduleCount; c4++) {
        for (var r4 = 0; r4 < moduleCount - 6; r4++) {
          if (qrCode.isDark(r4, c4) && !qrCode.isDark(r4 + 1, c4) && qrCode.isDark(r4 + 2, c4) &&
              qrCode.isDark(r4 + 3, c4) && qrCode.isDark(r4 + 4, c4) && !qrCode.isDark(r4 + 5, c4) &&
              qrCode.isDark(r4 + 6, c4)) lostPoint += 40;
        }
      }
      var darkCount = 0;
      for (var col2 = 0; col2 < moduleCount; col2++) {
        for (var row2 = 0; row2 < moduleCount; row2++) {
          if (qrCode.isDark(row2, col2)) darkCount++;
        }
      }
    ...
    make: function() {
      if (this.typeNumber < 1) {
        ...
        for (typeNumber = 1; typeNumber < 40; typeNumber++) {
          ...
          var length = 0;
          for (var j = 0; j < this.dataList.length; j++) length += this.dataList[j].getLength();
          if (length <= totalDataCount - 3) break;
        }
        this.typeNumber = typeNumber;
      }
  ...
  QRCodeModel.createBytes = function(buffer, rsBlocks) {
    ...
    for (var i2 = 0; i2 < maxDcCount; i2++) {
      for (var r2 = 0; r2 < rsBlocks.length; r2++) {
        if (i2 < dcdata[r2].length) data[index++] = dcdata[r2][i2];
      }
    }
    for (var i3 = 0; i3 < maxEcCount; i3++) {
      for (var r3 = 0; r3 < rsBlocks.length; r3++) {
        if (i3 < ecdata[r3].length) data[index++] = ecdata[r3][i3];
      }
    }
    return data;
  };

  // High Level QRCode Helper for Browsers
  function QRCode(targetEl, options) {
    if (typeof options === "string") options = { text: options };
    this.options = {
      width: (options && options.width) || 256,
      height: (options && options.height) || 256,
      colorDark: (options && options.colorDark) || "#000000",
      colorLight: (options && options.colorLight) || "#ffffff",
      correctLevel: (options && options.correctLevel !== undefined) ? options.correctLevel : QRErrorCorrectLevel.M,
      text: (options && options.text) || ""
    };
    this.targetEl = typeof targetEl === "string" ? document.getElementById(targetEl) : targetEl;
