...
const context={LAB_SESSIONS:sessions,location:{hash:'#'+session.id+'/'+(index+1)},localStorage:{getItem:()=>JSON.stringify({[session.id+'/'+index]:state,['intro:'+session.id]:true}),setItem(){}},document:{querySelector:element,getElementById:element,querySelectorAll:()=>[]},window:{addEventListener(){}},console};
...
vm.runInNewContext(fs.readFileSync(path.join(root,'onboarding.js'),'utf8'),context);
