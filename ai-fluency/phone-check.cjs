...
data.PHONE_JOURNEYS=data.window.PHONE_JOURNEYS;vm.runInNewContext(source('consequences.js'),data);
...
for(let n=0;n<8;n++){
 ...
 const state={version:2,started:true,step:n,answers:{},receipts:[],visited:['Browser:'+n,'Notes:'+n],played:{[n]:true}};
 const ctx={window:{resolveConsequenceStep:data.window.resolveConsequenceStep},PHONE_JOURNEYS:journeys,localStorage:{getItem:()=>JSON.stringify({[j.id]:state}),setItem(){}},document:{querySelector:el,getElementById:el,querySelectorAll:()=>[]},clearTimeout(){},setTimeout(){},matchMedia:()=>({matches:true}),console};
